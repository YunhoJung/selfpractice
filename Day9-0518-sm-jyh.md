...
l = ['{} x {} = {}'.format(x,y, x*y) x*y if y != 9 else '{} \n'.format(
    ...: x*y) for x in range(2,10) for y in range(2,10)]

 l = ['{} x {} = {}'.format(x,y, x*y if y != 9 else '{} \n'.format(x*y)) for x in range(2,10) for y in range(2,10)]
    
 실습 6번   
l2 = [x for x in range(100) if x % 7 == 0 or x % 9 == 0]

l2 = [x for x in range(100) if not x % 7 or not x % 9] // 0 == false 다른 숫자나 값은 true
...
def 함수명(매개변수[parameters]):
	동작
```
// 매개변수가 없는 함수 예를 들어 def func():
같은 경우 밑에 있는 동작내용에도 매개변수가 포함되어 있지않고 매개변수와 관련없는 동작이 포함된다.
#### 함수의 정의, 실행
...
```python
# 함수 정의때는 매개변수
def func(매개변수1, 매개변수2): // 매개변수를 활용한다
  ...
  
# 함수 호출시에는 인자
>>> func(인자1, 인자2) //인자를 전달한다
```

#### 리턴값이 없을 경우

함수에서 리턴해 주는 값이 없을 경우, 아무것도 없다는 뜻을 가진 `None`객체를 얻는다. 
//false는 거짓이라는 값을 가지는 것 모든 none객체는 독립적인 같은 id 값
...
def print_args(*args):
  print(args)
```
// arguments의 줄임 아무거나 써도되지만 관용적으로 많이 씀
#### 키워드인자 묶음
...
//타입값으로 정확히 보기. 함수는 유일하게 호출이 가능한 변수
